"""
Client helper functions for syncing with a self-hosted pacli server.
"""
...
logger = get_logger("pacli.sync_client")

SYNC_CONFIG_PATH = os.path.expanduser("~/.config/pacli/sync_config.json")
...
def resolve_server_params(server_url: str | None = None, token: str | None = None) -> tuple[str | None, str | None]:
    ...
    2. Environment variables (PACLI_SYNC_SERVER, PACLI_SYNC_TOKEN)
    3. Saved config (~/.config/pacli/sync_config.json)
    """
    config = get_sync_config()
    ...
    if not res_server:
        res_server = os.environ.get("PACLI_SYNC_SERVER") or config.get("server_url")
    if not res_token:
        res_token = os.environ.get("PACLI_SYNC_TOKEN") or config.get("token")
