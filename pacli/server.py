...
logger = get_logger("pacli.server")

SERVER_DIR = os.path.expanduser("~/.config/pacli/server")
...
class SyncServerDB:
    ...
    def create_token(self, name: str, role: str = "member") -> tuple[str, str]:
        ...
        Returns:
            (token_id, raw_token)
        """
        raw_token = f"pacli_tok_{secrets.token_urlsafe(32)}"
...
def create_sync_server_app(db: SyncServerDB | None = None) -> Flask:
    ...
    def health():
        return jsonify({"status": "ok", "service": "pacli-sync-server"})
    ...
    def push_blob(vault_name):
        ...
        user_name = request.headers.get("X-Pacli-User", default_user)
