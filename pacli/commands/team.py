...
Provides the `pacli team` command group for managing shared vaults,
team membership, and viewing audit logs.
"""

import click
import datetime
from ..vault import VaultManager, get_user_identity, set_user_identity
...
def create_vault(name, description):
    ...
    vm = VaultManager()
    try:
        meta = vm.create_vault(name, description=description, master_fernet=store.fernet)
        click.echo(f"\n✅ Vault '{name}' created!")
        ...
        click.echo(f"\n💡 Use 'pacli add --vault {name} ...' to add secrets to this vault.")
        click.echo(f"   Use 'pacli team add-member {name} <user-id>' to invite team members.")
...
def list_vaults():
    ...
    if not vaults:
        click.echo("📭 No vaults found. Create one with 'pacli team create-vault <name>'.")
        return
...
def add_member(vault_name, user_id, user_name, role):
    ...
    (they can find it with 'pacli team whoami').
    """
    store = SecretStore()
    store.require_fernet()
    ...
    vm = VaultManager()
    try:
        vm.add_member(vault_name, user_id, user_name, role, store.fernet)
