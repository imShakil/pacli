...
def sync_push(vault_name, target_path, server_url, token, sync_password):
    ...
    if server_url and not target_path:
        ...
        _push_to_server_handler(vault_name, server_url, token, store.fernet)
        return
    ...
    _push_to_filesystem_handler(vault_name, target_path, sync_password, store.fernet)


@sync.command("pull")
@click.argument("vault_name")
@click.option("--from", "-f", "source_path", default=None, help="Source directory path containing the .pacli file.")
@click.option("--server", "-s", "server_url", default=None, help="Sync server URL (overrides saved config).")
@click.option("--token", "token", default=None, help="Sync server token (overrides saved config).")
@click.option(
