"""
CLI commands for managing the self-hosted pacli sync server.
"""
...
logger = get_logger("pacli.server_cli")
...
def server_start(host, port, daemon):
    ...
    if daemon:
        ...
        cmd = [sys.executable, "-m", "pacli.commands.server", "_internal_run", "--host", host, "--port", str(port)]
...
def token_create(name, role):
    ...
    click.echo(f"   pacli sync config set --server <server_url> --token {raw_token}")
